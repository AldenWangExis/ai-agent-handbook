...
原项目按目录把三十章拆成了几十个文件，适合在 GitHub 上分章阅读。本仓库把它们合并成一份完整文档，方便离线阅读、整本传阅和导入阅读器：保留原书顺序与全部插图，附带可跳转的目录，并提供 Markdown、EPUB、PDF 三种格式。原项目 README 中的分章导航表格由下方目录代替，其余内容收在文末「附录：关于本书」。

- 原项目：<{UPSTREAM}>
- 本仓库：<{FORK}>
- 创建日期：{today}
- 篇幅：{sum(1 for e in BOOK if e['level'] == 1)} 个一级板块，{sum(1 for e in BOOK if e['level'] == 2)} 个章节条目
